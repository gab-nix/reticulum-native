...
typedef struct { const uint8_t *p; const uint8_t *end; } reader_t;

static uint32_t rotr32(uint32_t x, unsigned n) { return (x >> n) | (x << (32u-n)); }
static uint32_t load32be(const uint8_t *p) { return ((uint32_t)p[0]<<24)|((uint32_t)p[1]<<16)|((uint32_t)p[2]<<8)|p[3]; }
static void store32be(uint8_t *p, uint32_t x) { p[0]=(uint8_t)(x>>24); p[1]=(uint8_t)(x>>16); p[2]=(uint8_t)(x>>8); p[3]=(uint8_t)x; }
static void store64be(uint8_t *p, uint64_t x) { for (unsigned i=0;i<8;i++) p[7-i]=(uint8_t)(x>>(8*i)); }
...
typedef struct { uint32_t h[8]; uint8_t block[64]; size_t used; uint64_t bits; } sha_ctx;
static void sha_block(sha_ctx *c, const uint8_t *b) {
    uint32_t w[64],a,bv,d,e,f,g,h; uint32_t cc;
    for(unsigned i=0;i<16;i++) w[i]=load32be(b+4*i);
    for(unsigned i=16;i<64;i++){uint32_t s0=rotr32(w[i-15],7)^rotr32(w[i-15],18)^(w[i-15]>>3);uint32_t s1=rotr32(w[i-2],17)^rotr32(w[i-2],19)^(w[i-2]>>10);w[i]=w[i-16]+s0+w[i-7]+s1;}
    a=c->h[0];bv=c->h[1];cc=c->h[2];d=c->h[3];e=c->h[4];f=c->h[5];g=c->h[6];h=c->h[7];
    for(unsigned i=0;i<64;i++){uint32_t s1=rotr32(e,6)^rotr32(e,11)^rotr32(e,25);uint32_t ch=(e&f)^((~e)&g);uint32_t t1=h+s1+ch+sha_k[i]+w[i];uint32_t s0=rotr32(a,2)^rotr32(a,13)^rotr32(a,22);uint32_t maj=(a&bv)^(a&cc)^(bv&cc);uint32_t t2=s0+maj;h=g;g=f;f=e;e=d+t1;d=cc;cc=bv;bv=a;a=t1+t2;}
    c->h[0]+=a;c->h[1]+=bv;c->h[2]+=cc;c->h[3]+=d;c->h[4]+=e;c->h[5]+=f;c->h[6]+=g;c->h[7]+=h;
}
static void sha_init(sha_ctx *c){static const uint32_t iv[8]={0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19};memcpy(c->h,iv,sizeof iv);c->used=0;c->bits=0;}
static void sha_update(sha_ctx *c,const uint8_t *p,size_t n){while(n){size_t take=64-c->used;if(take>n)take=n;memcpy(c->block+c->used,p,take);c->used+=take;p+=take;n-=take;c->bits+=(uint64_t)take*8;if(c->used==64){sha_block(c,c->block);c->used=0;}}}
static void sha_final(sha_ctx *c,uint8_t out[32]){c->block[c->used++]=0x80;if(c->used>56){memset(c->block+c->used,0,64-c->used);sha_block(c,c->block);c->used=0;}memset(c->block+c->used,0,56-c->used);store64be(c->block+56,c->bits);sha_block(c,c->block);for(unsigned i=0;i<8;i++)store32be(out+4*i,c->h[i]);}
void lxmf_sha256(const uint8_t *data,size_t len,uint8_t digest[32]){sha_ctx c;sha_init(&c);if(len)sha_update(&c,data,len);sha_final(&c,digest);}

static bool put(writer_t *w,const void *p,size_t n){if(n>w->left)return false;memcpy(w->p,p,n);w->p+=n;w->left-=n;return true;}
static bool put_u8(writer_t *w,uint8_t x){return put(w,&x,1);}
static bool put_bin(writer_t *w,lxmf_slice_t s){
    if(s.len && !s.data)return false;
    if(s.len<=UINT8_MAX){uint8_t h[2]={0xc4,(uint8_t)s.len};if(!put(w,h,2))return false;}
    else if(s.len<=UINT16_MAX){uint8_t h[3]={0xc5,(uint8_t)(s.len>>8),(uint8_t)s.len};if(!put(w,h,3))return false;}
    else if(s.len<=UINT32_MAX){uint8_t h[5]={0xc6,(uint8_t)(s.len>>24),(uint8_t)(s.len>>16),(uint8_t)(s.len>>8),(uint8_t)s.len};if(!put(w,h,5))return false;}
    else return false;
    return put(w,s.data,s.len);
}
static bool put_double(writer_t *w,double d){uint64_t u;uint8_t b[9]={0xcb};memcpy(&u,&d,8);store64be(b+1,u);return put(w,b,9);}

static bool skip_obj(reader_t *r,unsigned depth){
    if(depth>LXMF_MAX_NESTING||r->p>=r->end)return false;uint8_t c=*r->p++;uint64_t n=0;size_t items=0;
    if(c<=0x7f||c>=0xe0||c==0xc0||c==0xc2||c==0xc3)return true;
    if((c&0xe0)==0xa0)n=c&31;else if((c&0xf0)==0x90)items=c&15;else if((c&0xf0)==0x80)items=2*(c&15);
    else switch(c){case 0xc4:case 0xd9:if(r->p+1>r->end)return false;n=*r->p++;break;case 0xc5:case 0xda:if(r->p+2>r->end)return false;n=((uint64_t)r->p[0]<<8)|r->p[1];r->p+=2;break;case 0xc6:case 0xdb:if(r->p+4>r->end)return false;n=load32be(r->p);r->p+=4;break;case 0xca:n=4;break;case 0xcb:n=8;break;case 0xcc:n=1;break;case 0xcd:n=2;break;case 0xce:n=4;break;case 0xcf:n=8;break;case 0xd0:n=1;break;case 0xd1:n=2;break;case 0xd2:n=4;break;case 0xd3:n=8;break;case 0xd4:n=2;break;case 0xd5:n=3;break;case 0xd6:n=5;break;case 0xd7:n=9;break;case 0xd8:n=17;break;case 0xdc:if(r->p+2>r->end)return false;items=((size_t)r->p[0]<<8)|r->p[1];r->p+=2;break;case 0xdd:if(r->p+4>r->end)return false;items=load32be(r->p);r->p+=4;break;case 0xde:if(r->p+2>r->end)return false;items=2u*(((size_t)r->p[0]<<8)|r->p[1]);r->p+=2;break;case 0xdf:if(r->p+4>r->end)return false;{uint32_t q=load32be(r->p);if(q>UINT32_MAX/2)return false;items=2u*(size_t)q;}r->p+=4;break;default:return false;}
    if(n){if(n>(uint64_t)(r->end-r->p))return false;r->p+=(size_t)n;}
    for(size_t i=0;i<items;i++)if(!skip_obj(r,depth+1))return false;return true;
}
...
static lxmf_status_t encode_payload(const lxmf_message_t *m,bool stamp,writer_t *w){
    ...
    if(m->fields_msgpack.len){reader_t rr={m->fields_msgpack.data,m->fields_msgpack.data+m->fields_msgpack.len};const uint8_t *p=rr.p;if(!skip_obj(&rr,0)||rr.p!=rr.end||((*p&0xf0)!=0x80&&*p!=0xde&&*p!=0xdf))return LXMF_ERR_FORMAT;if(!put(w,p,m->fields_msgpack.len))return LXMF_ERR_BOUNDS;}else if(!put_u8(w,0x80))return LXMF_ERR_BOUNDS;
    if(stamp){size_t n=m->stamp_len?m->stamp_len:LXMF_STAMP_LENGTH;if(n!=LXMF_STAMP_LENGTH&&n!=LXMF_POW_STAMP_LENGTH)return LXMF_ERR_FORMAT;lxmf_slice_t s={m->stamp,n};if(!put_bin(w,s))return LXMF_ERR_BOUNDS;}return LXMF_OK;
}
...
lxmf_status_t lxmf_unpack(const uint8_t *in,size_t len,lxmf_verify_fn verify,void *ctx,lxmf_message_t *m){
    ...
    if(verify){uint8_t pre[16+16+4096+32];size_t n=0;memcpy(pre+n,m->destination,16);n+=16;memcpy(pre+n,m->source,16);n+=16;memcpy(pre+n,payload,plen);n+=plen;memcpy(pre+n,m->message_id,32);n+=32;st=verify(ctx,m->source,pre,n,m->signature);if(st!=LXMF_OK)return LXMF_ERR_SIGNATURE;}return LXMF_OK;
}
...
bool lxmf_ticket_stamp_valid(const uint8_t stamp[16],const uint8_t ticket[16],const uint8_t id[32]){uint8_t want[16],difference=0;lxmf_ticket_stamp(ticket,id,want);for(unsigned i=0;i<16;i++)difference|=(uint8_t)(want[i]^stamp[i]);return difference==0;}

lxmf_status_t lxmf_identity_signer(void *identity,const uint8_t *data,size_t len,uint8_t signature[64]){if(!identity||(!data&&len)||!signature)return LXMF_ERR_ARGUMENT;return rns_identity_sign((const rns_identity *)identity,data,len,signature)?LXMF_OK:LXMF_ERR_CRYPTO;}
lxmf_status_t lxmf_identity_verifier(void *context,const uint8_t source[16],const uint8_t *data,size_t len,const uint8_t signature[64]){lxmf_identity_verifier_context_t *c=(lxmf_identity_verifier_context_t *)context;if(!c||!c->resolve||!source||(!data&&len)||!signature)return LXMF_ERR_ARGUMENT;const rns_identity *identity=c->resolve(c->resolve_context,source);if(!identity)return LXMF_ERR_SIGNATURE;return rns_identity_verify(identity,data,len,signature)?LXMF_OK:LXMF_ERR_SIGNATURE;}
