 ...
 * V5 put/replace and V2 delivery updates append a Unix-seconds identity
 * discovery deadline. All prior formats remain readable by this reader;
 * older binaries must not open upgraded journals. */
#define HEADER_SIZE 16u
#define PUT_FIXED 77u
#define PUT2_FIXED 80u
#define PUT3_FIXED 132u
#define PUT4_FIXED 134u
#define PUT5_FIXED 142u
#define STATUS_SIZE 33u
#define SIGNATURE_SIZE 33u
#define REMOVE_SIZE 32u
#define DELIVERY_SIZE 84u
#define DELIVERY2_SIZE 92u
#define MAX_PAYLOAD                                                            \
  (PUT5_FIXED + LXMF_STORE_MAX_CONTENT + LXMF_STORE_MAX_PACKED)
#define TYPE_PUT 1u
...
#define TYPE_PUT_V5 10u
#define TYPE_REPLACE_V5 11u
#define TYPE_DELIVERY_V2 12u
...
typedef struct {
  FILE *file;
  char path[LXMF_STORE_PATH_MAX + 1];
  index_entry index[LXMF_STORE_MAX_MESSAGES];
  size_t count;
} store_impl;
...
static bool valid_method(uint8_t method) {
...
static bool valid_delivery(const lxmf_delivery_metadata_t *delivery) {
  return delivery && delivery->desired_method >= LXMF_DELIVERY_METHOD_UNKNOWN &&
         delivery->desired_method <= LXMF_DELIVERY_METHOD_PROPAGATED &&
         delivery->actual_method >= LXMF_DELIVERY_METHOD_UNKNOWN &&
         delivery->actual_method <= LXMF_DELIVERY_METHOD_PROPAGATED &&
         valid_queue_reason((uint8_t)delivery->queue_reason) &&
         delivery->progress <= LXMF_DELIVERY_PROGRESS_COMPLETE;
}
static index_entry *find(store_impl *s, const uint8_t id[32]) {
  for (size_t i = 0; i < s->count; i++)
    if (memcmp(s->index[i].id, id, 32) == 0)
      return &s->index[i];
  return NULL;
}
static lxmf_status_t sync_file(FILE *f) {
  if (fflush(f) != 0)
    return LXMF_ERR_CRYPTO;
  return fsync(fileno(f)) == 0 ? LXMF_OK : LXMF_ERR_CRYPTO;
}
static lxmf_status_t append_record(FILE *f, uint8_t type,
                                   const uint8_t *payload, uint32_t n,
                                   uint64_t *offset) {
  if (fseek(f, 0, SEEK_END) != 0)
    return LXMF_ERR_CRYPTO;
  long pos = ftell(f);
  if (pos < 0 || (uint64_t)pos + HEADER_SIZE + n > LXMF_STORE_MAX_FILE_SIZE)
    return LXMF_ERR_BOUNDS;
  uint8_t h[HEADER_SIZE] = {'L', 'X', 'M', 'S', 1, type, 0, 0};
  put32(h + 8, n);
  put32(h + 12, crc32_bytes(payload, n));
  if (fwrite(h, 1, sizeof h, f) != sizeof h || fwrite(payload, 1, n, f) != n)
    return LXMF_ERR_CRYPTO;
  lxmf_status_t st = sync_file(f);
  if (st == LXMF_OK && offset)
    *offset = (uint64_t)pos;
  return st;
}
...
static void encode_delivery(uint8_t *payload,
  ...
  put64(payload + 52, delivery->identity_deadline);
}

static bool decode_delivery(const uint8_t *payload,
                            lxmf_delivery_metadata_t *delivery, bool extended) {
  ...
  if (extended) delivery->identity_deadline = get64(payload + 52);
  if (delivery->has_proof_id)
...
static bool same_delivery(const lxmf_delivery_metadata_t *left,
  ...
  return left->desired_method == right->desired_method &&
         ...
         left->identity_deadline == right->identity_deadline &&
         left->progress == right->progress &&
         left->has_proof_id == right->has_proof_id &&
         (!left->has_proof_id ||
          memcmp(left->proof_id, right->proof_id, LXMF_MESSAGE_ID_LENGTH) == 0);
}
...
static void encode_put(uint8_t payload[PUT5_FIXED],
                       const lxmf_store_message_t *m) {
  memcpy(payload, m->message_id, 32);
  memcpy(payload + 32, m->destination, 16);
  memcpy(payload + 48, m->source, 16);
  uint64_t bits;
  memcpy(&bits, &m->timestamp, 8);
  put64(payload + 64, bits);
...
static lxmf_status_t append_put_record(FILE *file,
                                       const lxmf_store_message_t *m,
                                       uint8_t type, uint64_t *offset) {
  uint8_t fixed[PUT5_FIXED];
  encode_put(fixed, m);
  uint32_t n = (uint32_t)(PUT5_FIXED + m->content.len + m->packed.len);
  ...
  if (fwrite(h, 1, sizeof h, file) != sizeof h ||
      ...
      (m->content.len &&
       fwrite(m->content.data, 1, m->content.len, file) != m->content.len) ||
      (m->packed.len &&
       fwrite(m->packed.data, 1, m->packed.len, file) != m->packed.len))
    return LXMF_ERR_CRYPTO;
...
static lxmf_status_t append_put(FILE *file, const lxmf_store_message_t *m,
                                uint64_t *offset) {
  return append_put_record(file, m, TYPE_PUT_V5, offset);
}

static index_entry *upsert(store_impl *s, const uint8_t *payload,
                           uint64_t offset, uint16_t fixed,
                           uint32_t content_len, uint32_t packed_len) {
  index_entry *e = find(s, payload);
  if (e)
    return e;
  if (s->count >= LXMF_STORE_MAX_MESSAGES)
    return NULL;
  e = &s->index[s->count++];
  memset(e, 0, sizeof *e);
...
static void drop_index(store_impl *s, size_t i) {
  if (i + 1 < s->count)
    memmove(&s->index[i], &s->index[i + 1],
            (s->count - i - 1) * sizeof s->index[0]);
  s->count--;
...
static lxmf_status_t scan(store_impl *s) {
  ...
  for (;;) {
    ...
    if (~crc != get32(h + 12))
      goto recover;
    if (h[5] == TYPE_PUT) {
      if (n < PUT_FIXED || n - PUT_FIXED > LXMF_STORE_MAX_CONTENT ||
          get32(payload + 73) != n - PUT_FIXED || !valid_status(payload[72]))
        goto recover;
      index_entry *e = upsert(s, payload, good, (uint16_t)PUT_FIXED,
                              n - (uint32_t)PUT_FIXED, 0u);
      if (!e)
        return LXMF_ERR_BOUNDS;
      e->status = payload[72];
      e->signature = (uint8_t)LXMF_SIGNATURE_VERIFIED;
    } else if (h[5] == TYPE_PUT_V2) {
      if (n < PUT2_FIXED || !valid_status(payload[72]) ||
          !valid_signature(payload[77]))
        goto recover;
      ...
      if (content_len > LXMF_STORE_MAX_CONTENT ||
          ...
          n != PUT2_FIXED + content_len + packed_len)
        goto recover;
      index_entry *e = upsert(s, payload, good, (uint16_t)PUT2_FIXED,
      ...
      e->signature = payload[77];
               ...
               h[5] == TYPE_REPLACE_V4 || h[5] == TYPE_PUT_V5 ||
               h[5] == TYPE_REPLACE_V5) {
      bool wide = h[5] != TYPE_PUT_V3;
      bool extended = h[5] == TYPE_PUT_V5 || h[5] == TYPE_REPLACE_V5;
      bool replacement = h[5] == TYPE_REPLACE_V4 || h[5] == TYPE_REPLACE_V5;
      uint16_t fixed = extended ? PUT5_FIXED : wide ? PUT4_FIXED : PUT3_FIXED;
      ...
      if (content_len > LXMF_STORE_MAX_CONTENT ||
          ...
          !decode_delivery(payload + (wide ? 82 : 80), &delivery, extended))
        goto recover;
      if (replacement && !find(s, payload))
        goto recover;
      index_entry *e = upsert(s, payload, good, fixed, content_len, packed_len);
      if (!e)
        return LXMF_ERR_BOUNDS;
      if (replacement) {
        e->offset = good;
      ...
      if (n != STATUS_SIZE || !valid_status(payload[32]))
        goto recover;
      index_entry *e = find(s, payload);
      if (e)
        e->status = payload[32];
    } else if (h[5] == TYPE_SIGNATURE) {
      if (n != SIGNATURE_SIZE || !valid_signature(payload[32]))
        goto recover;
      index_entry *e = find(s, payload);
      if (e)
        e->signature = payload[32];
    } else if (h[5] == TYPE_REMOVE) {
      if (n != REMOVE_SIZE)
        goto recover;
      index_entry *e = find(s, payload);
      if (e)
        drop_index(s, (size_t)(e - s->index));
    } else {
      bool extended = h[5] == TYPE_DELIVERY_V2;
      if (n != (extended ? DELIVERY2_SIZE : DELIVERY_SIZE))
        goto recover;
      lxmf_delivery_metadata_t delivery;
      if (!decode_delivery(payload + 32, &delivery, extended))
        goto recover;
      index_entry *e = find(s, payload);
      if (e)
        e->delivery = delivery;
...
recover:
  clearerr(s->file);
  if (ftruncate(fileno(s->file), (off_t)good) != 0)
    return LXMF_ERR_CRYPTO;
...
lxmf_status_t lxmf_store_open(lxmf_store_t *store, const char *path) {
  if (!store || !path || store->implementation ||
      strlen(path) > LXMF_STORE_PATH_MAX)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = calloc(1, sizeof *s);
  if (!s)
    return LXMF_ERR_BOUNDS;
  memcpy(s->path, path, strlen(path) + 1);
  s->file = fopen(path, "a+b");
  if (!s->file) {
    free(s);
    return LXMF_ERR_CRYPTO;
  }
  store->implementation = s;
  lxmf_status_t st = scan(s);
  if (st != LXMF_OK) {
...
size_t lxmf_store_count(const lxmf_store_t *store) {
  return store && store->implementation
             ? ((store_impl *)store->implementation)->count
             : 0;
}

static size_t unverified_total(const store_impl *s) {
  size_t n = 0;
  for (size_t i = 0; i < s->count; i++)
    if (s->index[i].signature != (uint8_t)LXMF_SIGNATURE_VERIFIED)
      n++;
  return n;
}
static size_t unverified_from(const store_impl *s, const uint8_t source[16]) {
  size_t n = 0;
  for (size_t i = 0; i < s->count; i++)
    if (s->index[i].signature != (uint8_t)LXMF_SIGNATURE_VERIFIED &&
        memcmp(s->index[i].source, source, 16) == 0)
      n++;
  return n;
}
...
size_t lxmf_store_unverified_count(const lxmf_store_t *store) {
  return store && store->implementation
             ? unverified_total(store->implementation)
             : 0;
}
...
static lxmf_status_t remove_at(store_impl *s, size_t i) {
  ...
  lxmf_status_t st =
      append_record(s->file, TYPE_REMOVE, payload, REMOVE_SIZE, NULL);
  if (st != LXMF_OK)
    return st;
  drop_index(s, i);
  return LXMF_OK;
...
static lxmf_status_t enforce_unverified_caps(store_impl *s,
...
lxmf_status_t lxmf_store_put(lxmf_store_t *store, const lxmf_store_message_t *m,
                             bool *inserted) {
  if (inserted)
    *inserted = false;
  if (!store || !store->implementation || !m ||
      m->content.len > LXMF_STORE_MAX_CONTENT ||
      (m->content.len && !m->content.data) ||
      m->packed.len > LXMF_STORE_MAX_PACKED ||
  ...
  if (s->count >= LXMF_STORE_MAX_MESSAGES)
    return LXMF_ERR_BOUNDS;
  uint8_t payload[PUT5_FIXED];
  encode_put(payload, m);
  uint64_t offset;
  lxmf_status_t st = append_put(s->file, m, &offset);
  if (st != LXMF_OK)
    return st;
  index_entry *e = upsert(s, payload, offset, (uint16_t)PUT5_FIXED,
                          (uint32_t)m->content.len, (uint32_t)m->packed.len);
  if (!e)
    return LXMF_ERR_BOUNDS;
...
lxmf_status_t lxmf_store_update_status(lxmf_store_t *store,
                                       const uint8_t id[32],
                                       lxmf_delivery_status_t status) {
  if (!store || !store->implementation || !id || !valid_status((uint8_t)status))
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
    return LXMF_ERR_FORMAT;
  if (e->status == (uint8_t)status)
    return LXMF_OK;
  uint8_t p[STATUS_SIZE];
  memcpy(p, id, 32);
  p[32] = (uint8_t)status;
  lxmf_status_t st = append_record(s->file, TYPE_STATUS, p, sizeof p, NULL);
  if (st == LXMF_OK)
    e->status = (uint8_t)status;
  return st;
}
lxmf_status_t lxmf_store_update_signature(lxmf_store_t *store,
                                          const uint8_t id[32],
                                          lxmf_signature_state_t state) {
  if (!store || !store->implementation || !id ||
      !valid_signature((uint8_t)state))
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
    return LXMF_ERR_FORMAT;
  if (e->signature == (uint8_t)state)
    return LXMF_OK;
  uint8_t p[SIGNATURE_SIZE];
  memcpy(p, id, 32);
  p[32] = (uint8_t)state;
  lxmf_status_t st = append_record(s->file, TYPE_SIGNATURE, p, sizeof p, NULL);
  if (st == LXMF_OK)
    e->signature = (uint8_t)state;
  return st;
}
lxmf_status_t
lxmf_store_update_delivery(lxmf_store_t *store, const uint8_t id[32],
                           const lxmf_delivery_metadata_t *delivery) {
  if (!store || !store->implementation || !id || !valid_delivery(delivery))
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
    return LXMF_ERR_FORMAT;
  if (same_delivery(&e->delivery, delivery))
    return LXMF_OK;
  uint8_t p[DELIVERY2_SIZE];
  memcpy(p, id, 32);
  encode_delivery(p + 32, delivery);
  lxmf_status_t st = append_record(s->file, TYPE_DELIVERY_V2, p, sizeof p, NULL);
  if (st == LXMF_OK)
    e->delivery = *delivery;
  return st;
}
lxmf_status_t lxmf_store_remove(lxmf_store_t *store, const uint8_t id[32]) {
  if (!store || !store->implementation || !id)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
    return LXMF_ERR_FORMAT;
  return remove_at(s, (size_t)(e - s->index));
}

static lxmf_status_t seek_body(store_impl *s, const index_entry *e,
                               uint64_t skip) {
  uint64_t at = e->offset + HEADER_SIZE + skip;
  if (at > (uint64_t)LONG_MAX)
    return LXMF_ERR_BOUNDS;
  return fseek(s->file, (long)at, SEEK_SET) == 0 ? LXMF_OK : LXMF_ERR_CRYPTO;
}

lxmf_status_t lxmf_store_read(lxmf_store_t *store, const uint8_t id[32],
                              lxmf_store_message_t *m, uint8_t *content,
                              size_t cap) {
  if (!store || !store->implementation || !id || !m)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
    return LXMF_ERR_FORMAT;
  if (e->content_len > cap || (e->content_len && !content))
    return LXMF_ERR_BOUNDS;
  lxmf_status_t st = seek_body(s, e, 0);
  if (st != LXMF_OK)
    return st;
  uint8_t fixed[PUT5_FIXED];
  if (fread(fixed, 1, e->fixed, s->file) != e->fixed)
    return LXMF_ERR_CRYPTO;
  memset(m, 0, sizeof *m);
  ...
  if (e->content_len &&
      fread(content, 1, e->content_len, s->file) != e->content_len)
    return LXMF_ERR_CRYPTO;
  return LXMF_OK;
}

lxmf_status_t lxmf_store_read_packed(lxmf_store_t *store, const uint8_t id[32],
                                     uint8_t *packed, size_t capacity,
                                     size_t *packed_len) {
  if (!store || !store->implementation || !id || !packed || !packed_len)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
  ...
  if (e->packed_len == 0u)
    return LXMF_ERR_FORMAT;
  if (e->packed_len > capacity)
    return LXMF_ERR_BOUNDS;
  lxmf_status_t st = seek_body(s, e, (uint64_t)e->fixed + e->content_len);
  if (st != LXMF_OK)
    return st;
  if (fread(packed, 1, e->packed_len, s->file) != e->packed_len)
    return LXMF_ERR_CRYPTO;
  *packed_len = e->packed_len;
  return LXMF_OK;
}

lxmf_status_t lxmf_store_packed_size(lxmf_store_t *store, const uint8_t id[32],
                                     size_t *packed_len) {
  if (!store || !store->implementation || !id || !packed_len)
    return LXMF_ERR_ARGUMENT;
  *packed_len = 0;
...
lxmf_status_t lxmf_store_update_packed(lxmf_store_t *store,
  ...
  store_impl *s = store->implementation;
  uint64_t offset;
  status = append_put_record(s->file, &message, TYPE_REPLACE_V5, &offset);
  if (status != LXMF_OK)
    return status;
  uint8_t fixed[PUT5_FIXED];
  encode_put(fixed, &message);
  index_entry *entry =
      upsert(s, fixed, offset, PUT5_FIXED, (uint32_t)message.content.len,
             (uint32_t)packed_len);
  if (!entry)
    return LXMF_ERR_BOUNDS;
  entry->offset = offset;
  entry->fixed = PUT5_FIXED;
...
lxmf_status_t lxmf_store_read_delivery(lxmf_store_t *store,
                                       const uint8_t id[32],
                                       lxmf_delivery_metadata_t *delivery) {
  if (!store || !store->implementation || !id || !delivery)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  index_entry *e = find(s, id);
  if (!e)
...
lxmf_status_t lxmf_store_list(lxmf_store_t *store, lxmf_store_list_fn cb,
                              void *ctx) {
  if (!store || !store->implementation || !cb)
    return LXMF_ERR_ARGUMENT;
  store_impl *s = store->implementation;
  uint8_t content[LXMF_STORE_MAX_CONTENT];
  for (size_t i = 0; i < s->count; i++) {
    ...
    lxmf_status_t st =
        lxmf_store_read(store, s->index[i].id, &m, content, sizeof content);
    if (st != LXMF_OK)
      return st;
    if (!cb(ctx, &m))
      break;
...
lxmf_status_t lxmf_store_compact(lxmf_store_t *store) {
  ...
  for (size_t i = 0; i < s->count && st == LXMF_OK; i++) {
    ...
    if (s->index[i].packed_len) {
      ...
      st = lxmf_store_read_packed(store, s->index[i].id, packed,
                                  s->index[i].packed_len, &packed_len);
      if (st != LXMF_OK) {
        free(packed);
        break;
      }
