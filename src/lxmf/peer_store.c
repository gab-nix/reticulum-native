...
#define MAX_PAYLOAD                                                            \
  (LXMF_PEER_STORE_MAX_PEERS * (RECORD_FIXED + LXMF_PEER_NAME_MAX +            \
                                LXMF_PEER_NOTE_MAX + LXMF_PEER_DRAFT_MAX))
...
static bool valid_peer(const lxmf_peer_t *p) {
  return p && p->display_name_len <= LXMF_PEER_NAME_MAX &&
         p->note_len <= LXMF_PEER_NOTE_MAX &&
         p->draft_len <= LXMF_PEER_DRAFT_MAX &&
         p->display_name[p->display_name_len] == '\0' &&
         p->note[p->note_len] == '\0' && p->draft[p->draft_len] == '\0' &&
         valid_utf8((const uint8_t *)p->display_name, p->display_name_len) &&
         valid_utf8((const uint8_t *)p->note, p->note_len) &&
         valid_utf8((const uint8_t *)p->draft, p->draft_len) &&
         p->trust <= LXMF_PEER_TRUST_UNTRUSTED &&
         p->propagation <= LXMF_PEER_PROPAGATION_PREFERRED;
}
static ptrdiff_t find_peer(const peer_store_impl *s,
                           const uint8_t address[16]) {
  for (size_t i = 0; i < s->count; i++)
    if (memcmp(s->peers[i].address, address, 16) == 0)
      return (ptrdiff_t)i;
  return -1;
}
...
static lxmf_status_t decode_file(const char *path, lxmf_peer_t *peers,
  ...
  if (!f)
    return errno == ENOENT ? LXMF_ERR_FORMAT : LXMF_ERR_CRYPTO;
  uint8_t h[HEADER_SIZE];
  lxmf_status_t st = LXMF_ERR_FORMAT;
  if (fread(h, 1, sizeof h, f) != sizeof h || memcmp(h, "LXPEERS\0", 8) != 0 ||
      get16(h + 10) != HEADER_SIZE)
    goto done;
  uint16_t version = get16(h + 8);
  if (version != 1u && version != LXMF_PEER_STORE_VERSION)
    goto done;
  ...
  if (n > LXMF_PEER_STORE_MAX_PEERS || payload_len > MAX_PAYLOAD)
    goto done;
  ...
  for (uint32_t i = 0; i < n; i++) {
    ...
    p->draft_len = dl;
    if (!valid_peer(p) ||
        find_peer(&(peer_store_impl){.peers = peers, .count = i}, p->address) >=
            0) {
      free(payload);
      goto done;
    }
...
static lxmf_status_t sync_parent(const char *path) {
  char dir[LXMF_PEER_STORE_PATH_MAX + 1];
  ...
  int rc = fsync(fd);
  close(fd);
  return rc == 0 ? LXMF_OK : LXMF_ERR_CRYPTO;
}

lxmf_status_t lxmf_peer_store_open(lxmf_peer_store_t *store, const char *path) {
  if (!store || !path || store->implementation ||
      strlen(path) > LXMF_PEER_STORE_PATH_MAX)
    return LXMF_ERR_ARGUMENT;
  peer_store_impl *s = calloc(1, sizeof *s);
  if (!s)
    return LXMF_ERR_BOUNDS;
  s->peers = calloc(LXMF_PEER_STORE_MAX_PEERS, sizeof *s->peers);
  if (!s->peers) {
    free(s);
    return LXMF_ERR_BOUNDS;
  }
  ...
  lxmf_status_t main_st = decode_file(path, s->peers, &s->count);
  if (main_st != LXMF_OK) {
    size_t recovered = 0;
    lxmf_status_t tmp_st = decode_file(tmp, s->peers, &recovered);
    if (tmp_st == LXMF_OK) {
...
size_t lxmf_peer_store_count(const lxmf_peer_store_t *store) {
  return store && store->implementation
             ? ((peer_store_impl *)store->implementation)->count
             : 0;
}
lxmf_status_t lxmf_peer_store_get(const lxmf_peer_store_t *store,
                                  const uint8_t address[16],
                                  lxmf_peer_t *peer) {
  if (!store || !store->implementation || !address || !peer)
    return LXMF_ERR_ARGUMENT;
  peer_store_impl *s = store->implementation;
  ptrdiff_t i = find_peer(s, address);
  if (i < 0)
...
lxmf_status_t lxmf_peer_store_put(lxmf_peer_store_t *store,
  ...
  if (inserted)
    *inserted = false;
  if (!store || !store->implementation || !valid_peer(peer))
    return LXMF_ERR_ARGUMENT;
  peer_store_impl *s = store->implementation;
  ptrdiff_t i = find_peer(s, peer->address);
  if (i < 0) {
    if (s->count >= LXMF_PEER_STORE_MAX_PEERS)
      return LXMF_ERR_BOUNDS;
    i = (ptrdiff_t)s->count++;
    if (inserted)
      *inserted = true;
...
lxmf_status_t lxmf_peer_store_remove(lxmf_peer_store_t *store,
                                     const uint8_t address[16], bool *removed) {
  if (removed)
    *removed = false;
  if (!store || !store->implementation || !address)
    return LXMF_ERR_ARGUMENT;
  peer_store_impl *s = store->implementation;
  ptrdiff_t i = find_peer(s, address);
  if (i < 0)
    return LXMF_OK;
  s->peers[i] = s->peers[s->count - 1];
  s->count--;
  if (removed)
    *removed = true;
  return LXMF_OK;
...
lxmf_status_t lxmf_peer_store_list(const lxmf_peer_store_t *store,
                                   lxmf_peer_store_list_fn cb, void *ctx) {
...
lxmf_status_t lxmf_peer_store_save(lxmf_peer_store_t *store) {
  ...
  for (size_t i = 0; i < s->count; i++) {
    ...
    off += RECORD_FIXED + p->display_name_len + p->note_len + p->draft_len;
  }
  ...
  if (fclose(f) != 0)
    st = LXMF_ERR_CRYPTO;
  if (st == LXMF_OK && rename(tmp, s->path) != 0)
    st = LXMF_ERR_CRYPTO;
  if (st == LXMF_OK)
    st = sync_parent(s->path);
  return st;
}
