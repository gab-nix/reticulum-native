...
enum ticket_kind { TICKET_OUTBOUND = 1, TICKET_INBOUND = 2,
...
static void put64(uint8_t *p, uint64_t v) {
    for (size_t i = 0u; i < 8u; ++i) p[7u - i] = (uint8_t)(v >> (8u * i));
}
...
static lxmf_status_t save(lxmf_ticket_store_t *store) {
    ...
    uint8_t header[TICKET_FILE_HEADER] = {'L', 'X', 'T', 'K',
                                          TICKET_FILE_VERSION, 0, 0, 0};
    ...
    if (length < 0 || (size_t)length >= sizeof temporary) return LXMF_ERR_BOUNDS;
    FILE *file = fopen(temporary, "w+b");
    if (file == NULL) return LXMF_ERR_CRYPTO;
    ...
    if (fclose(file) != 0 && status == LXMF_OK) status = LXMF_ERR_CRYPTO;
    if (status == LXMF_OK && rename(temporary, store->path) != 0)
        status = LXMF_ERR_CRYPTO;
    if (status != LXMF_OK) (void)unlink(temporary);
    return status;
}

static lxmf_status_t load(lxmf_ticket_store_t *store) {
    FILE *file = fopen(store->path, "rb");
    if (file == NULL) return errno == ENOENT ? LXMF_OK : LXMF_ERR_CRYPTO;
    uint8_t header[TICKET_FILE_HEADER];
    lxmf_status_t status = LXMF_ERR_FORMAT;
    if (fread(header, 1u, sizeof header, file) != sizeof header ||
        memcmp(header, "LXTK", 4u) != 0 ||
        header[4] != TICKET_FILE_VERSION) goto done;
    uint32_t count = get32(header + 8u);
    if (count > LXMF_TICKET_STORE_MAX_ENTRIES) goto done;
    size_t payload_length = (size_t)count * TICKET_FILE_RECORD;
    uint8_t payload[LXMF_TICKET_STORE_MAX_ENTRIES * TICKET_FILE_RECORD];
    if (payload_length != 0u &&
        fread(payload, 1u, payload_length, file) != payload_length) goto done;
    if (fgetc(file) != EOF || crc32_bytes(payload, payload_length) !=
                                  get32(header + 12u)) goto done;
    for (uint32_t i = 0u; i < count; ++i) {
        const uint8_t *record = payload + (size_t)i * TICKET_FILE_RECORD;
        if (!valid_kind(record[0])) goto done;
...
lxmf_status_t lxmf_ticket_store_open(lxmf_ticket_store_t **output,
    ...
    if (output == NULL || path == NULL || *path == '\0' ||
        strlen(path) > LXMF_TICKET_STORE_PATH_MAX) return LXMF_ERR_ARGUMENT;
    *output = NULL;
    lxmf_ticket_store_t *store = calloc(1u, sizeof *store);
    if (store == NULL) return LXMF_ERR_BOUNDS;
    memcpy(store->path, path, strlen(path) + 1u);
    lxmf_status_t status = load(store);
    if (status != LXMF_OK) { free(store); return status; }
    *output = store;
    return LXMF_OK;
}
...
static stored_ticket_t *find_one(lxmf_ticket_store_t *store, uint8_t kind,
    ...
    for (size_t i = 0u; i < store->count; ++i)
        if (same_peer(&store->entries[i], kind, peer)) return &store->entries[i];
    return NULL;
}
static stored_ticket_t *append_entry(lxmf_ticket_store_t *store) {
    if (store->count == LXMF_TICKET_STORE_MAX_ENTRIES) return NULL;
...
lxmf_status_t lxmf_ticket_store_issue(lxmf_ticket_store_t *store,
    ...
    if (last != NULL && (last->time > now ||
        now - last->time < LXMF_TICKET_INTERVAL_SECONDS))
...
lxmf_status_t lxmf_ticket_store_remember_outbound(
    ...
    if (store == NULL || peer == NULL || ticket == NULL ||
        ticket->expires_at <= now) return LXMF_ERR_ARGUMENT;
    stored_ticket_t *entry = find_one(store, TICKET_OUTBOUND, peer);
    bool appended = false;
    if (entry == NULL) { entry = append_entry(store); appended = true; }
    if (entry == NULL) return LXMF_ERR_BOUNDS;
    stored_ticket_t previous = *entry;
    entry->kind = TICKET_OUTBOUND; memcpy(entry->peer, peer, 16u);
    entry->time = ticket->expires_at;
    memcpy(entry->ticket, ticket->ticket, LXMF_TICKET_LENGTH);
    lxmf_status_t status = save(store);
    if (status != LXMF_OK) {
        if (appended) --store->count; else *entry = previous;
    }
    return status;
}
...
lxmf_status_t lxmf_ticket_store_get_outbound(lxmf_ticket_store_t *store,
    ...
    if (store == NULL || peer == NULL || ticket == NULL)
        return LXMF_ERR_ARGUMENT;
    stored_ticket_t *entry = find_one(store, TICKET_OUTBOUND, peer);
    if (entry == NULL || entry->time <= now) return LXMF_ERR_PENDING;
    ticket->expires_at = entry->time;
    memcpy(ticket->ticket, entry->ticket, LXMF_TICKET_LENGTH);
    return LXMF_OK;
}
...
lxmf_status_t lxmf_ticket_store_get_inbound(
    ...
    if (store == NULL || peer == NULL || ticket_count == NULL ||
        (capacity != 0u && tickets == NULL)) return LXMF_ERR_ARGUMENT;
    *ticket_count = 0u;
    for (size_t i = 0u; i < store->count; ++i) {
        stored_ticket_t *entry = &store->entries[i];
        if (!same_peer(entry, TICKET_INBOUND, peer) || entry->time <= now)
            continue;
        if (*ticket_count == capacity) return LXMF_ERR_BOUNDS;
        tickets[*ticket_count].expires_at = entry->time;
        memcpy(tickets[*ticket_count].ticket, entry->ticket,
               LXMF_TICKET_LENGTH);
        ++*ticket_count;
    }
    return LXMF_OK;
...
lxmf_status_t lxmf_ticket_store_stamp_outbound(
    lxmf_ticket_store_t *store, const uint8_t peer[16], uint64_t now,
    const uint8_t message_id[32], uint8_t stamp[16]) {
    if (message_id == NULL || stamp == NULL) return LXMF_ERR_ARGUMENT;
    lxmf_ticket_entry_t ticket;
    lxmf_status_t status = lxmf_ticket_store_get_outbound(store, peer, now,
                                                           &ticket);
...
lxmf_status_t lxmf_ticket_store_validate_inbound(
    lxmf_ticket_store_t *store, const uint8_t peer[16], uint64_t now,
    const uint8_t message_id[32], const uint8_t stamp[16]) {
...
lxmf_status_t lxmf_ticket_store_mark_delivered(lxmf_ticket_store_t *store,
                                               const uint8_t peer[16],
                                               uint64_t now) {
    if (store == NULL || peer == NULL) return LXMF_ERR_ARGUMENT;
    stored_ticket_t *entry = find_one(store, TICKET_LAST_DELIVERY, peer);
    bool appended = false;
    if (entry == NULL) { entry = append_entry(store); appended = true; }
    if (entry == NULL) return LXMF_ERR_BOUNDS;
    stored_ticket_t previous = *entry;
    entry->kind = TICKET_LAST_DELIVERY; memcpy(entry->peer, peer, 16u);
    entry->time = now; memset(entry->ticket, 0, sizeof entry->ticket);
    lxmf_status_t status = save(store);
    if (status != LXMF_OK) {
        if (appended) --store->count; else *entry = previous;
    }
...
lxmf_status_t lxmf_ticket_store_cleanup(lxmf_ticket_store_t *store,
    ...
    for (size_t i = store->count; i > 0u; --i) {
        ...
        if (expired) { remove_at(store, i - 1u); changed = true; }
    }
