...
static int openssl_sha256(const uint8_t *data, size_t length, uint8_t out[32]) {
    if (!out || !valid_buffer(data, length)) return 0;
    return SHA256(data, length, out) != NULL;
}

static int openssl_hmac_sha256(const uint8_t *key, size_t key_length, const uint8_t *data,
...
static int openssl_hkdf_sha256(const uint8_t *input, size_t input_length, const uint8_t *salt,
    ...
    if (!openssl_hmac_sha256(salt, salt_length, input, input_length, prk)) return 0;
    while (produced < out_length) {
        ...
        if (!openssl_hmac_sha256(prk, sizeof(prk), scratch, message_length, block)) goto fail;
...
static int openssl_random_bytes(uint8_t *out, size_t length) {
...
static int openssl_x25519_generate(uint8_t private_key[32], uint8_t public_key[32]) { return generate_raw(EVP_PKEY_X25519, private_key, public_key); }
static int openssl_x25519_public_from_private(const uint8_t private_key[32], uint8_t public_key[32]) { return raw_public_from_private(EVP_PKEY_X25519, private_key, public_key); }
static int openssl_ed25519_generate(uint8_t private_key[32], uint8_t public_key[32]) { return generate_raw(EVP_PKEY_ED25519, private_key, public_key); }
static int openssl_ed25519_public_from_private(const uint8_t private_key[32], uint8_t public_key[32]) { return raw_public_from_private(EVP_PKEY_ED25519, private_key, public_key); }

static int openssl_x25519_exchange(const uint8_t private_key[32], const uint8_t peer_public[32], uint8_t shared[32]) {
...
static int openssl_ed25519_sign(const uint8_t private_key[32], const uint8_t *message, size_t message_length, uint8_t signature[64]) {
...
static int openssl_ed25519_verify(const uint8_t public_key[32], const uint8_t *message, size_t message_length, const uint8_t signature[64]) {
...
static int openssl_token_encrypt(const uint8_t key[64], const uint8_t *plaintext, size_t plaintext_length,
    ...
    if (!openssl_random_bytes(out, 16)) return 0;
    ...
    if (ctx && EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), NULL, key + 32, out) == 1 &&
        ...
        openssl_hmac_sha256(key, 32, out, 16u + (size_t)n + (size_t)final_n, mac)) {
...
static int openssl_token_decrypt(const uint8_t key[64], const uint8_t *token, size_t token_length,
                      uint8_t *out, size_t out_capacity, size_t *out_length) {
...
static int provider_sha256(void *context, const uint8_t *data, size_t length,
                           uint8_t out[32]) {
    (void)context; return openssl_sha256(data, length, out);
}
static int provider_hmac(void *context, const uint8_t *key, size_t key_length,
                         const uint8_t *data, size_t data_length, uint8_t out[32]) {
    (void)context; return openssl_hmac_sha256(key, key_length, data, data_length, out);
}
static int provider_hkdf(void *context, const uint8_t *input, size_t input_length,
                         const uint8_t *salt, size_t salt_length,
                         const uint8_t *info, size_t info_length,
                         uint8_t *out, size_t out_length) {
    (void)context;
    return openssl_hkdf_sha256(input, input_length, salt, salt_length,
                               info, info_length, out, out_length);
}
static int provider_random(void *context, uint8_t *out, size_t length) {
    (void)context; return openssl_random_bytes(out, length);
}
static int provider_x_generate(void *context, uint8_t private_key[32],
                               uint8_t public_key[32]) {
    (void)context; return openssl_x25519_generate(private_key, public_key);
}
static int provider_x_public(void *context, const uint8_t private_key[32],
                             uint8_t public_key[32]) {
    (void)context; return openssl_x25519_public_from_private(private_key, public_key);
}
static int provider_x_exchange(void *context, const uint8_t private_key[32],
                               const uint8_t peer_public[32], uint8_t shared[32]) {
    (void)context; return openssl_x25519_exchange(private_key, peer_public, shared);
}
static int provider_ed_generate(void *context, uint8_t private_key[32],
                                uint8_t public_key[32]) {
    (void)context; return openssl_ed25519_generate(private_key, public_key);
}
static int provider_ed_public(void *context, const uint8_t private_key[32],
                              uint8_t public_key[32]) {
    (void)context; return openssl_ed25519_public_from_private(private_key, public_key);
}
static int provider_ed_sign(void *context, const uint8_t private_key[32],
                            const uint8_t *message, size_t message_length,
                            uint8_t signature[64]) {
    (void)context;
    return openssl_ed25519_sign(private_key, message, message_length, signature);
}
static int provider_ed_verify(void *context, const uint8_t public_key[32],
                              const uint8_t *message, size_t message_length,
...
static int provider_token_encrypt(void *context, const uint8_t key[64],
                                  ...
                                  uint8_t *out, size_t out_capacity, size_t *out_length) {
    (void)context;
    return openssl_token_encrypt(key, plaintext, plaintext_length,
                                 out, out_capacity, out_length);
}
static int provider_token_decrypt(void *context, const uint8_t key[64],
