...
int main(int argc,char **argv){if(argc<2){usage(stderr);return EXIT_USAGE;}if(!strcmp(argv[1],"init")&&argc==3)return cmd_init(argv[2]);if(!strcmp(argv[1],"address")&&argc==3)return cmd_address(argv[2]);if(!strcmp(argv[1],"pack")&&argc==6)return cmd_pack(argv[2],argv[3],argv[4],argv[5]);if(!strcmp(argv[1],"unpack")&&argc==3)return cmd_unpack(argv[2]);if(!strcmp(argv[1],"history")&&argc==3)return cmd_history(argv[2]);if(!strcmp(argv[1],"repl")&&argc==5)return cmd_repl(argv[2],argv[3],argv[4]);if(!strcmp(argv[1],"send-file")&&argc==3){(void)argv;fprintf(stderr,"network delivery runtime is not implemented; file was not sent\n");return EXIT_UNAVAILABLE;}usage(stderr);return EXIT_USAGE;}
