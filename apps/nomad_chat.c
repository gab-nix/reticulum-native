...
int main(int argc,char **argv){if(argc<2){usage(stderr);return EXIT_USAGE;}if(!strcmp(argv[1],"init")&&argc==3)return cmd_init(argv[2]);if(!strcmp(argv[1],"address")&&argc==3)return cmd_address(argv[2]);if(!strcmp(argv[1],"pack")&&argc==6)return cmd_pack(argv[2],argv[3],argv[4],argv[5]);if(!strcmp(argv[1],"unpack")&&argc==3)return cmd_unpack(argv[2]);if(!strcmp(argv[1],"history")&&argc==3)return cmd_history(argv[2]);if(!strcmp(argv[1],"repl")&&argc==5)return cmd_repl(argv[2],argv[3],argv[4]);if(!strcmp(argv[1],"send-udp")&&argc==8)return cmd_send_udp(argv[2],argv[3],argv[4],argv[5],argv[6],argv[7]);if(!strcmp(argv[1],"receive-udp")&&(argc==5||argc==6))return cmd_receive_udp(argv[2],argv[3],argv[4],argc==6?argv[5]:NULL);if(!strcmp(argv[1],"tui")){int dump=argc>2&&!strcmp(argv[2],"--dump-ui");int base=dump?3:2;if(argc!=base+2&&argc!=base+3){usage(stderr);return EXIT_USAGE;}if(access(argv[base],R_OK)!=0){fprintf(stderr,"cannot open identity %s: %s\n",argv[base],strerror(errno));return EXIT_DATA;}const char *destination=argc==base+3?argv[base+2]:NULL;int result=dump?nomad_tui_dump(argv[base],argv[base+1],destination,stdout):nomad_tui_run_destination(argv[base],argv[base+1],destination);return result==0?0:EXIT_DATA;}if(!strcmp(argv[1],"send-file")&&argc==3){(void)argv;fprintf(stderr,"network delivery runtime is not implemented; file was not sent\n");return EXIT_UNAVAILABLE;}usage(stderr);return EXIT_USAGE;}
