...
typedef struct { uint8_t destination[16], next_hop[16], public_key[64]; uint8_t app_data[RNS_NODE_APP_DATA_MAX]; size_t app_data_length; uint64_t announce_timebase; uint8_t hops; uint64_t interface_id; int32_t gravity; double seen_at, expires_at; bool reachable, propagation, has_ratchet; char name[64]; } rns_node_record;
