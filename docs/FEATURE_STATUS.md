...
| Native Micron browser | WIP | A caller-polled remote fetch state machine performs route discovery, authenticated link establishment, and both packet-backed and resource-backed page retrieval, with cancellation, retained-page failures, and tested absolute-path normalization for `<hash>:/page/...` URLs. Real pages from live Nomad nodes now load. The Micron renderer still only understands a minimal markup subset, so genuine Micron formatting and `` `[label`target] `` links render as raw text and are not yet navigable. Complete markup, forms, caching and upstream page tests remain. |
