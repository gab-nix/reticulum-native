...
| Heltec persistent chats and quick replies | WIP | Application-owned chat-store core supports eight senders with eight records each, 384-byte bounded text, message IDs, timestamps and delivery-state storage through borrowed transactional storage. Candidate updates commit to RAM only after successful persistence; duplicate IDs are ignored and pending messages/chats cannot be evicted or deleted. `test_chat_store` covers reopen, failed writes, deduplication, capacity, pending protection and corrupt versions. Dedicated-partition backend, OLED wiring, confirmations, receipt-driven updates and outgoing delivery remain unfinished. Existing 24 KiB NVS is not repurposed for chat history; the running firmware still has only volatile previews and no user-message sending |
