...
| Heltec persistent chats and quick replies | WIP | Application-owned chat-store core supports eight senders with eight records each, 384-byte bounded text, message IDs, timestamps and delivery-state storage through borrowed transactional storage. Candidate updates commit to RAM only after successful persistence; duplicate IDs are ignored and pending messages/chats cannot be evicted or deleted. `test_chat_store` covers reopen, failed writes, deduplication, capacity, pending protection and corrupt versions. A separate 64 KiB dual-sector journal now supplies CRC-checked records, generation selection, readback verification and deletion tombstones; simulated torn replacement recovers the older record in `test_chat_journal`. The ESP adapter targets only the reserved storage partition and refuses foreign data; NVS/identity boundaries remain unchanged. Runtime/OLED wiring, confirmations, receipt-driven updates and outgoing delivery remain unfinished; physical power-loss testing is pending. Running firmware still has volatile previews only |
