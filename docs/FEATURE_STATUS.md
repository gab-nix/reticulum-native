...
| Heltec channel activity history | WIP | Channel menu displays three aligned RX/TX/CAD-busy timelines over 60 seconds from optional scheduler telemetry, using fixed storage and a one-second refresh. Each visible cell covers three seconds; missing samples, counter resets and clock regression are unknown, not idle. Rolling airtime use is shown against the current fixed 36000 ms budget, plus queue size and cumulative duty deferrals. `test_channel_view` covers combined events, rollover, gaps, reset, unavailable telemetry and bounded rendering; menu tests cover Channel selection and wrap. RSSI/SNR display and physical graph validation remain unfinished. Not a spectrum analyzer or channel occupancy estimate |
