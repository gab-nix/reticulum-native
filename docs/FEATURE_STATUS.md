...
| Heltec OLED and receive-only hardware diagnostics | IMPLEMENTED | ESP-IDF I2C adapter and application loop connect the OLED core and SX1262 owner task. Tests cover large diagnostic glyph bounds, display/radio failure isolation, bounded receive draining and no linked send API. ESP-IDF 5.5.4 builds and physical OLED/RX initialization pass with stable heap and zero transmitted frames. Large-text visual acceptance, raw-frame exchange and protocol dispatch remain separate gates |
