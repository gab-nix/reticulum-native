...
| Native Micron browser | WIP | A caller-polled remote fetch state machine now performs route discovery, authenticated link establishment and bounded packet-response page parsing, with cancellation, retained-page failures, and tested absolute-path normalization for `<hash>:/page/...` URLs. Resource-backed responses are detected and reported explicitly rather than timing out, but cannot be received: live Nomad nodes answer `/page/index.mu` with a RESOURCE_ADV, so real pages do not load. Resource transfer, forms, caching, complete markup and upstream page tests remain. |
