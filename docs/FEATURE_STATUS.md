...
| Heltec Home dashboard and faster navigation | WIP | Startup Home displays received complete-packet and successful-transmission counts, scheduler payload-byte totals, current/minimum heap and pending TX. `test_home_view` covers unavailable statistics, counter saturation and bounded formatting. Hold selection is now 500 ms with threshold regression coverage. CPU explicitly displays unavailable until runtime sampling is integrated; activity history, RSSI/SNR and immediate input-triggered rendering remain unfinished. Transport remains disabled |
