...
| Heltec Home dashboard and faster navigation | WIP | Startup Home displays received complete-packet and successful-transmission counts, scheduler payload-byte totals, current/minimum heap and pending TX. `test_home_view` covers unavailable statistics, counter saturation and bounded formatting. Hold selection is 500 ms; navigation triggers immediate rendering. ESP-IDF runtime snapshots sample both idle-task counters once per second into a five-interval weighted CPU average; unavailable/incomplete/discontinuous snapshots remain explicitly unavailable. `test_cpu_usage` covers one-core load, fully busy/idle, warmup, counter rollover and invalid snapshots. Fixed 24-task snapshot capacity fails unavailable on overflow. Hardware accuracy, activity history and RSSI/SNR remain acceptance work; transport remains disabled |
| Heltec channel activity history | IMPLEMENTED | Channel menu displays aligned RX/TX/CAD-busy timelines over 60 seconds from optional scheduler telemetry, using fixed storage and a one-second refresh. Each visible cell covers three seconds; missing samples, counter resets and clock regression are unknown, not idle. Rolling airtime use is shown against the fixed 36000 ms budget, plus queue size and cumulative duty deferrals. Last received PHY-frame RSSI in dBm and SNR in dB are labelled LAST; before reception they are explicitly unknown. `test_channel_view` covers combined events, rollover, gaps, reset, unavailable telemetry, signed signal values and bounded rendering; scheduler tests verify signal retention; menu tests cover selection and wrap. Physical graph and signal validation remain acceptance gates. Not a spectrum analyzer or channel occupancy estimate |
| Heltec persistent chats and quick replies | WIP | Application-owned chat-store core supports eight senders with eight records each, 384-byte bounded text, message IDs, timestamps and delivery-state storage through borrowed transactional storage. Candidate updates commit to RAM only after successful persistence; duplicate IDs are ignored and pending messages/chats cannot be evicted or deleted. `test_chat_store` covers reopen, failed writes, deduplication, capacity, pending protection and corrupt versions. A separate 64 KiB dual-sector journal supplies CRC-checked records, generation selection, readback verification and deletion tombstones; simulated torn replacement recovers the older record in `test_chat_journal`. The ESP adapter targets only the reserved storage partition and refuses foreign data; NVS/identity boundaries remain unchanged. Runtime/OLED wiring, per-message journal redesign, confirmations, outgoing delivery and physical power-loss testing remain unfinished. Running firmware still has volatile previews only |
| Embedded durable receive gate and unverified events | IMPLEMENTED | Optional packet-endpoint acceptance hook exposes borrowed raw packets and explicit verified/unverified signature status. Verified messages enter replay state and receive proofs only after hook success; persistence failure permits retry. Unknown signers never receive proofs or legacy verified callbacks. `test_lxmf_packet_node` covers unknown event, announce revalidation, refused save with no proof/replay acceptance, and successful retransmission. Firmware now gates verified receipts on chat-store saves and restores bounded previews at startup; Home/Messages display storage errors without stopping radio polling. `test_chat_journal` integrates sender records, reopen and duplicate suppression through the flash simulator. Clear View clears only volatile previews, not saved history. Unknown-sender archival, per-message journal migration, grouped navigation, confirmed deletion and restart signature revalidation remain unwired; physical persistence verification is pending |
