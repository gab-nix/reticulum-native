...
| LXMF delivery and propagation | WIP | Opportunistic encoding now matches the pinned Python layout: the outer Reticulum destination supplies the first 16 packed bytes and encryption covers only the remaining LXMF representation, with an independent decrypted-wire regression test. Messages are routed by learned paths and unknown senders are retained in a bounded durable unverified queue for later revalidation. C-to-C UDP tests exist, but Python interoperability is not yet recorded. Direct/resource delivery, packet proofs, receipt handling, ratchets, stamp policy, retry backoff and propagation sync remain. |
