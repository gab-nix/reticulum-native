...
static uint16_t port(void) {
  ...
  assert(rns_udp_local_address(e, &a) == RNS_OK);
  rns_udp_endpoint_destroy(e);
  return a.port;
}
static void config(rns_config_t *c, uint16_t listen, uint16_t forward) {
  ...
  strcpy(i->name, "propagation-test");
  i->type = RNS_CONFIG_UDP;
  i->type_set = true;
  i->enabled = true;
  strcpy(i->listen_ip, "127.0.0.1");
  strcpy(i->forward_ip, "127.0.0.1");
  i->listen_port = listen;
  i->forward_port = forward;
}
...
static void accepted(rns_runtime_destination_t *d, rns_runtime_link_t *l,
                     void *context) {
  host_t *h = context;
  (void)d;
  assert(h->link_count < 4);
  h->links[h->link_count++] = l;
}
static bool accept_resource(rns_runtime_link_t *l,
                            const rns_resource_advertisement_t *a,
                            void *context) {
  (void)l;
  (void)a;
  (void)context;
...
static const rns_identity *verify_sender(void *context,
                                         const uint8_t hash[16]) {
  host_t *h = context;
  return memcmp(hash, h->sender_hash, 16) == 0 ? h->sender : NULL;
}
static void received(rns_runtime_link_t *l, const uint8_t hash[32],
                     rns_status_t status, const uint8_t *data, size_t length,
                     void *context) {
  host_t *h = context;
  (void)l;
  (void)hash;
  ...
  h->valid = status == RNS_OK &&
             lxmf_pn_upload_decode(data, length, &upload) == LXMF_OK &&
             upload.count == 1;
  if (!h->valid)
    return;
  lxmf_slice_t item = upload.messages[0];
  if (item.len <= 16u + RNS_TOKEN_OVERHEAD + 32u ||
      memcmp(item.data, h->recipient_hash, 16) != 0) {
    h->valid = false;
    return;
  }
  size_t transient_length = item.len - 32u;
  uint8_t transient[32];
  lxmf_sha256(item.data, transient_length, transient);
  if (lxmf_pow_stamp_validate_expanded(
          transient, 1, LXMF_PROPAGATION_STAMP_WORKBLOCK_ROUNDS,
          item.data + transient_length, NULL) != LXMF_OK) {
    h->valid = false;
    return;
  }
  uint8_t plain[4096], packed[4112];
  size_t plain_length = 0;
  if (!rns_identity_decrypt(h->recipient, item.data + 16, transient_length - 16,
                            plain, sizeof plain, &plain_length)) {
    h->valid = false;
    return;
  }
  memcpy(packed, item.data, 16);
  memcpy(packed + 16, plain, plain_length);
  lxmf_message_t message;
  lxmf_identity_verifier_context_t verifier = {.resolve = verify_sender,
                                               .resolve_context = h};
  h->valid = lxmf_unpack(packed, plain_length + 16, lxmf_identity_verifier,
                         &verifier, &message) == LXMF_OK &&
             message.content.len == 2048 &&
             memcmp(message.destination, h->recipient_hash, 16) == 0;
}
static void queue(lxmf_store_t *store, const rns_identity *sender,
                  const uint8_t destination[16], uint8_t id[32]) {
  uint8_t content[2048], packed[2304];
  for (size_t i = 0; i < sizeof content; i++)
    content[i] = (uint8_t)(i * 71u + i / 3u);
  ...
  memcpy(message.source, source, 16);
  message.timestamp = 123;
  message.content = (lxmf_slice_t){content, sizeof content};
  size_t packed_length;
  assert(lxmf_pack(&message, lxmf_identity_signer, (void *)sender, packed,
                   sizeof packed, &packed_length) == LXMF_OK);
  assert(lxmf_unpack(packed, packed_length, NULL, NULL, &decoded) == LXMF_OK);
  memcpy(id, decoded.message_id, 32);
  lxmf_store_message_t stored = {0};
  memcpy(stored.message_id, id, 32);
  memcpy(stored.destination, destination, 16);
  memcpy(stored.source, source, 16);
  stored.timestamp = 123;
...
static void pump(rns_runtime_t *a, rns_runtime_t *b, lxmf_router_t *router,
  ...
  for (size_t i = 0; i < 15000; i++) {
    ...
    assert(lxmf_store_read(router->config.store, id, &stored, content,
...
int main(void) {
  ...
  rns_identity sender, recipient, node;
  ...
  const char *propagation[] = {"propagation"};
  uint8_t node_hash[16];
  assert(rns_destination_hash(&node, "lxmf", propagation, 1, node_hash));
  rns_runtime_link_options_t links = {.resource_accept_callback =
                                          accept_resource,
                                      .resource_receive_callback = received,
                                      .max_incoming_resource_size = 4096,
                                      .callback_context = &host};
  rns_runtime_destination_t *registration = NULL;
  assert(rns_runtime_register_link_destination(server, node_hash, &node, &links,
                                               accepted, &host,
                                               &registration) == RNS_OK);
  assert(rns_runtime_announce(server, &node, "lxmf", propagation, 1, NULL, 0) ==
         RNS_OK);
  size_t processed;
  ...
  lxmf_router_config_t options = {.identity = &sender,
                                  .store = &store,
                                  .runtime = client,
                                  ...
                                  .monotonic_clock_context = &host,
                                  .preferred_delivery_method =
                                      LXMF_DELIVERY_METHOD_PROPAGATED,
                                  .propagation_node_identity = &node,
  ...
  assert(lxmf_router_set_propagation_node(&router, &node, node_hash, 1u) ==
         LXMF_OK);
  uint8_t wrong_hash[16];
  memcpy(wrong_hash, node_hash, sizeof wrong_hash);
  wrong_hash[0] ^= 1u;
  ...
  queue(&store, &sender, host.recipient_hash, id);
  options.store = &store;
  options.propagation_stamp_cost = 254;
  assert(lxmf_router_init(&router, &options) == LXMF_OK);
  lxmf_router_poll_result_t result;
  assert(lxmf_router_poll(&router, 1, &result) == LXMF_OK);
  ...
  queue(&store, &sender, host.recipient_hash, id);
  options.store = &store;
  options.propagation_node_identity = NULL;
  options.propagation_stamp_cost = 0;
  assert(lxmf_router_init(&router, &options) == LXMF_OK);
  assert(lxmf_router_poll(&router, 1, &result) == LXMF_OK &&
         result.deferred == 1);
  assert(lxmf_store_read_delivery(&store, id, &metadata) == LXMF_OK);
  assert(metadata.queue_reason == LXMF_QUEUE_REASON_PROPAGATION_NODE &&
         metadata.attempts == 0);
  metadata.attempts = 2;
  ...
  rns_identity missing;
  assert(rns_identity_generate(&missing));
  uint8_t missing_hash[16];
  assert(rns_destination_hash(&missing, "lxmf", propagation, 1, missing_hash));
  fresh_store(path, &store);
  queue(&store, &sender, host.recipient_hash, id);
  options.store = &store;
  options.propagation_node_identity = &missing;
  options.monotonic_clock = NULL;
  options.monotonic_clock_context = NULL;
  memcpy(options.propagation_node_destination, missing_hash, 16);
  options.propagation_stamp_cost = 1;
  options.resource_timeout_seconds = 0.01;
  assert(lxmf_router_init(&router, &options) == LXMF_OK);
  for (size_t i = 0; i < 1000; i++) {
    assert(rns_runtime_poll(client, 16, &processed) == RNS_OK);
    assert(lxmf_router_poll(&router, 2, &result) == LXMF_OK);
    assert(lxmf_store_read_delivery(&store, id, &metadata) == LXMF_OK);
    if (metadata.queue_reason == LXMF_QUEUE_REASON_RETRY_EXHAUSTED)
      break;
  ...
  queue(&store, &sender, host.recipient_hash, id);
  options.store = &store;
  options.propagation_node_identity = &node;
  memcpy(options.propagation_node_destination, node_hash, 16);
  options.resource_timeout_seconds = 0;
  assert(lxmf_store_update_status(&store, id, LXMF_DELIVERY_SENDING) ==
         LXMF_OK);
  assert(lxmf_router_init(&router, &options) == LXMF_OK);
  lxmf_store_message_t recovered;
  uint8_t content[LXMF_STORE_MAX_CONTENT];
  assert(lxmf_store_read(&store, id, &recovered, content, sizeof content) ==
         LXMF_OK);
  ...
  lxmf_store_close(&store);
  unlink(path);
  for (size_t i = 0; i < host.link_count; i++)
    rns_runtime_link_destroy(host.links[i]);
