...
#include <assert.h>
#include <string.h>
static bool snapshot(void *context,size_t slot,uint8_t address[16],unsigned *state) {
    (void)context; if(slot>=2) return false;
    memset(address,0,16); address[15]=(uint8_t)(slot+1U); *state=slot==1?2:0; return true;
}
...
int main(void) {
    ...
    assert(v.node_key[15]==2 && strstr(lines[5],"LINKED"));
    assert(!strcmp(lines[6],"ENCRYPTED LINK"));
