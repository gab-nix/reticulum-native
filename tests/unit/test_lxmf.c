...
int main(void){
    ...
    uint8_t packed[512];size_t packed_len=0;assert(lxmf_pack(&m,fake_sign,NULL,packed,sizeof packed,&packed_len)==LXMF_OK);assert(packed[96]==0x94&&packed[97]==0xcb);lxmf_message_t u;assert(lxmf_unpack(packed,packed_len,fake_verify,NULL,&u)==LXMF_OK);assert(u.timestamp==1.5&&u.title.len==2&&memcmp(u.title.data,"hi",2)==0&&u.content.len==5&&memcmp(u.content.data,"hello",5)==0&&u.fields_msgpack.len==sizeof fields);
